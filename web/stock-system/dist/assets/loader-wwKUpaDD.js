import { q as authedFetch } from "./index-dXDZ0nX5.js";
...
const CACHE_KEYS = {
  SIGNALS: "signals",
  // signals 实际键: signals:v1 / signals:v2（loader.js 拼后缀）
...
function setStrategyVersion(version) {
  localStorage.setItem(STRATEGY_VERSION_KEY, version);
  clearAllCache();
}
async function loadSignals(forceRefresh = false, version = getStrategyVersion()) {
  const result = await smartFetch(
    `${CACHE_KEYS.SIGNALS}:${version}`,
    // 缓存键加 version 维度
    async () => {
  ...
  return {
    date: (api == null ? void 0 : api.date) || "",
    version: (api == null ? void 0 : api.version) || version,
    strategyName: (api == null ? void 0 : api.strategy_name) || "",
    data: (api == null ? void 0 : api.data) || []
  };
}
async function loadStrategyVersions(forceRefresh = false) {
  const result = await smartFetch(
...
export {
  ...
  getChangeClass as e,
  formatMoney as f,
  getStrategyVersion as g,
  formatPercent as h,
  loadPositions as i,
  loadTrades as j,
  loadStats as k,
  loadStrategyVersions as l,
  getCacheTTL as m,
  getMarketState as n,
  loadNavHistory as o,
  setStrategyVersion as p,
  loader as q,
  smartFetch as s
};
