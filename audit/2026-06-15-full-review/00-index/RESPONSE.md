...
**建议改法**：
- **选 A：立刻删**。filter-repo 已完成，操作不可逆，备份无用
- `rm -rf .git.backup-before-filter-repo/`
- ⏱ 1分钟
- ⚠️ 风险：如果 filter-repo 出错需要回滚会丢失。但 filter-repo 已经成功（证据：TUSHARE_TOKEN 已轮换），回滚需求极低
...
**建议改法**：
- 改为白名单 `allow_origins=["https://auto-claw.top", "https://stock.auto-claw.top", "https://www.auto-claw.top"]`
- ⏱ 15分钟
- ⚠️ 风险：本地开发需要额外加 `http://localhost:3000` 等
...
实际数据下载脚本：scripts/daily_data_fetch.py, scripts/daily_download.py, scripts/update_data.py
```

**建议改法**：
- CRON.md 和 MAINTENANCE.md 中 `daily_download.sh` 改为 `daily_data_fetch.py`
- ⏱ 10分钟
- ⚠️ 风险：无
...
**建议改法**：
- feedbacks.json 加入 .gitignore
- 已 commit 的历史用 filter-repo 清除（或接受风险，因为 IP 是公网 IP 且已过期）
- 长期改用 SQLite 存储
- ⏱ 30分钟
- ⚠️ 风险：filter-repo 操作需谨慎
...
**建议改法**：
- 统一返回 `{"error": "服务器内部错误"}`，详细错误只写日志
- ⏱ 1小时
- ⚠️ 风险：调试时不方便，建议保留 logger.info 记录完整错误
